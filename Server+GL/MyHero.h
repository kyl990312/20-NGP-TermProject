...
public:
	...
	float addedY = 0.0f;
	glm::vec3 soul_pos{ 0.0f,0.0f,400.0f };
	float log_speed = 0;
	//bool fall_into_river = false;
	HeroState _state = HeroState::Idle;
	glm::vec3 prevPosition = { 0.0f,0.0f,0.0f };
	glm::vec3 otherHero[2];

public:
	bool check_collision(glm::vec3 obs_pos,int obs_tag);
	//void jump();
	void Jump(int tag, glm::vec3* obs_pos, int obs_cnt1);
	void Float();
	void Die();
	//bool check_death(glm::vec3 obs_pos, int obs_tag);
	void update(int tag, glm::vec3* obs_pos, int obs_cnt1);
	bool CheckCollistionWithHero(glm::vec3 position, float size);
